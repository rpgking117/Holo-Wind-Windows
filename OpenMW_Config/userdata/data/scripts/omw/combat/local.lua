...
local function spawnBloodEffect(position)
    ...
    -- Guard against missing Blood_Model GMSTs (not present in vanilla Morrowind.esm).
    -- Without this check the concatenation below raises a Lua error and prevents
    -- Actor._onHit from running, causing every hit to deal zero damage.
    local gmstModel = core.getGMST(bloodEffectModel)
    if not gmstModel then return end

    -- TODO: implement a Misc::correctMeshPath equivalent instead?
    -- All it ever does it append 'meshes\\' though
    bloodEffectModel = 'meshes/'..gmstModel
    ...
    if not bloodTexture then return end
...
return {
    ...
    interface = {
        ...
        --- Computes this character's armor rating.
        -- You can override this to determine which armor skill an item uses.
        -- Note that this interface function is read by the engine to update the UI.
        -- This function can still be overridden same as any other interface, but must not call any functions or interfaces that modify anything.
